...
def update_account_details():
    ...
    try:
        ...
        return jsonify({'error': 'Error updating account details'}), 500
...
def change_password():
    ...
    if not current_password or not new_password or not confirm_new_password:
        return jsonify({'error': 'Current password, new password, and confirm new password are required'}), 400
...
def add_address():
    if 'user_id' not in session:
        return jsonify({'error': 'User must be logged in to add an address'}), 401
    ...
    address_type = data.get('address_type')  # 'shipping', 'billing', or 'both'

    if not address_line_1 or not city or not state or not zip_code or not phone_number or not address_type:
        return jsonify({'error': 'All address fields and address type are required'}), 400
    ...
    try:
        ...
        return jsonify({'address_id': str(address_id)}), 201
    except Exception as e:
        logging.error(f"Error adding address: {e}")
        return jsonify({'error': 'Error adding address'}), 500

#get all addreses
@app.route('/api/account/addresses', methods=['GET'])
def get_addresses():
    if 'user_id' not in session:
        return jsonify({'error': 'User must be logged in to view addresses'}), 401
    ...
    try:
        ...
        return jsonify({'error': 'Error retrieving addresses'}), 500

#addreses update route
@app.route('/api/account/address/<address_id>', methods=['PUT'])
def update_address(address_id):
    if 'user_id' not in session:
        return jsonify({'error': 'User must be logged in to update an address'}), 401
    ...
    try:
        ...
        return jsonify({'error': 'Error updating address'}), 500

#Addresses delete route
@app.route('/api/account/address/<address_id>', methods=['DELETE'])
def delete_address(address_id):
    if 'user_id' not in session:
        return jsonify({'error': 'User must be logged in to delete an address'}), 401
...
# Route to create a Razorpay order or Cash on Delivery order
@app.route('/api/create_order', methods=['POST'])
def create_order():
    if 'user_id' not in session:
        return jsonify({'error': 'User must be logged in to create an order'}), 401
    ...
    try:
        ...
        return jsonify({'order_id': str(order_id), 'razorpay_order_id': order_data.get('razorpay_order_id')}), 201
...
def checkout():
    ...
    try:
        ...
        return jsonify({'message': 'Order created successfully', 'order': str(order_id), 'razorpay_order': razorpay_order}), 201
